...
def Layout(layout ,img_shape, target_w, target_h):
    layout_viz, _, _ = draw_boundary_from_cor_id(layout, img_shape)    
    layout_viz = cv2.resize(layout_viz, (target_w,target_h), interpolation=cv2.INTER_NEAREST)    
...
def convert_3_classes(layout):
    _layout = torch.zeros(layout.shape)
    _layout[layout==2]=0;_layout[layout==3]=1;_layout[layout==1]=2 #Old
...
def draw_boundary_from_cor_id(cor_id, img_src):
    ...
    for dx, dy in [[-1, 0], [1, 0], [0, 0], [0, 1], [0, -1]]:
        panoEdgeC[np.clip(rs + dx, 0, im_h - 1), np.clip(cs + dy, 0, im_w - 1), 0] = 255
