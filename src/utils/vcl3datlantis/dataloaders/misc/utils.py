...
COLORS = uc.unique_color_rgb()
COLORS_LEN = len(COLORS)

def showImages(downscale_factor = 1, **kwargs):
    save = "save" in kwargs
    for k in kwargs:
        if k == "save":
            continue
        ...
        if downscale_factor != 1:
            img = cv2.resize(img, (0,0), fx = 1/downscale_factor, fy = 1/downscale_factor)            
        cv2.imshow(k, img)
        if save:
            prefix = kwargs["save"]
            cv2.imwrite(os.path.join(OUTPUT_PATH, f"{prefix}_{k}.png"), img)
    if not save:
        cv2.waitKey(0)

def colorizeLabels(labels : np.ndarray) -> np.ndarray:
    img_shape = (labels.shape[0], labels.shape[1], 3)
    ret_img = np.array([COLORS[xi] if xi < COLORS_LEN else (0,0,0) for xi in labels.flatten()],
