...
def initialize(args): #TODO: add visdom count as argument       
    ...
    gpus = [int(id) for id in args.gpu_id.split(',') if int(id) >= 0]
    device = torch.device("cuda:{}" .format(gpus[0]) if torch.cuda.is_available() and len(gpus) > 0 and gpus[0] >= 0 else "cpu")
    print("Training {0} for {1} iterations using a batch size of {2} on {3}".format(args.name, args.epochs, args.batch_size, device))
    # create visualizer
