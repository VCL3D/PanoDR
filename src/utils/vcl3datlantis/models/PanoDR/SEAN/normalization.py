...
class ACE(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc, device, ACE_Name=None, status='train', spade_params=None, use_rgb=True):
        super().__init__()
    ...
    def forward(self, x, segmap, style_codes=None, obj_dic=None):
        ...
        added_noise = (torch.randn(x.shape[0], x.shape[3], x.shape[2], 1).to(self.noise_var.device) * self.noise_var).transpose(1, 3)
...
class SPADE(nn.Module):
    def __init__(self, norm_nc, label_nc):
        ...
        # The dimension of the intermediate embedding space. Yes, hardcoded.
        nhidden = 128
        #self.device = torch.device("cuda:" + str(int(1))) 
        pw = ks // 2
