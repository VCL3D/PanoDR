...
class BaseModel(nn.Module):
    ...
    def load_networks(self, load_path):
        for i in range(len(self.model_names)):
            if isinstance(self.model_names[i], str):
                net = getattr(self, 'net' + self.model_names[i])
                if isinstance(net, torch.nn.DataParallel):
                    net = net.module
                print('loading the model from %s' % load_path[i])
                # if you are using PyTorch newer than 0.4 (e.g., built from
                # GitHub source), you can remove str() on self.device
                state_dict = torch.load(load_path[i], map_location="cuda:{}".format(self.opt.gpu_id))
